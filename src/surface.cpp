...
void Surface::Plot( float x, float y, Pixel c )
{
	...
	if((ix >= 0) && (iy >= 0) && (ix < m_Width) && (iy < m_Height))
		m_Buffer[ix + iy * m_Pitch] = c;

	// auto plot = [this](int x, int y, Pixel c) {
	// 	if((x >= 0) && (y >= 0) && (x < m_Width) && (y < m_Height))
	// 		m_Buffer[x + y * m_Pitch] = c;
	// };

	// auto blend = [this](int x, int y, Pixel c, float f) {
	// 	byte r = (byte)((c & 0xE0) >> 5);
	// 	byte g = (byte)((c & 0x1C) >> 2);
	// 	byte b = (byte)(c & 0x03);   

	// 	if(!((x >= 0) && (y >= 0) && (x < m_Width) && (y < m_Height)) || f == 1.)
	// 		return c;

	// 	Pixel c1 = m_Buffer[x + y * m_Pitch];
	// 	byte r1 = (byte)((c1 & 0xE0) >> 5);
	// 	byte g1 = (byte)((c1 & 0x1C) >> 2);
	// 	byte b1 = (byte)(c1 & 0x03);   

	// 	byte nr = c * r + (1. - c) * r1;
	// 	byte ng = c * g + (1. - c) * g1;
	// 	byte nb = c * b + (1. - c) * b1;

	// 	return (Pixel)((nr << 5) | (ng << 2) | nb);
	// };

	// // how much of the pixel is actually in (x+1, y+1)
	// float dx = fmodf(x, 1), dy = fmodf(y, 1);

	// Pixel cxy = blend(x, y, c, 1);
	// // TODO: cx1y, cxy1, cx1y1
...
void Sprite::DrawScaled( int a_X, int a_Y, int a_Width, int a_Height, Surface* a_Target )
{
	...
	for ( int x = 0; x < a_Width; x++ ) for ( int y = 0; y < a_Height; y++ )
	{
		...
		// adapted from https://stackoverflow.com/a/12043228/16166019
		// fixes dirty-looking almost-black outlines around sprites
		uint r = (color & RedMask) >> 16;
		uint g = (color & GreenMask) >> 8;
